...
app = Flask(__name__)
app.secret_key = 'grievance_chatbot_secret_key'  # Needed for sessions
...
def add_cors_headers(response):
    response.headers["Access-Control-Allow-Origin"] = "http://127.0.0.1:5500"  # Allow specific origin
...
connection_pool = MySQLConnectionPool(pool_name="grievance_pool",
...
def admin_login():
    ...
    if admin:
        session['admin_id'] = admin['id']  # Set admin_id in the session
...
@app.route('/api/admin/complaints', methods=['GET','OPTIONS'])
def get_all_complaints():
    ...
    cursor.close()
    conn.close()
    response = make_response(jsonify({"success": True, "complaints": complaints}))
    response.headers['Cache-Control'] = 'no-store'  # prevent 304
